...
# 流程：
#   1. 安装/复用固定版本 Hugo
#   2. 根据主站 related_laws 生成法条库反向文章链接
#   3. 安装/检查 VuePress 依赖（存在 lock 时优先 npm ci）
#   4. VuePress build → law-site/docs/.vuepress/dist/
#   5. 复制 VuePress 输出到 public/laws/
#   6. 生成 laws/sitemap.xml
#   7. Hugo build
...
echo "==> 1. 生成法条页相关文章反向链接"
python3 "$ROOT_DIR/scripts/generate-law-backlinks.py"

echo "==> 2. 安装/检查 VuePress 依赖"
cd "$LAW_SITE_DIR"
if [ -f "package-lock.json" ]; then
    ...
    if [ ! -d "node_modules" ] || [ "package.json" -nt "node_modules/.package-lock.json" ]; then
        echo "    未发现 package-lock.json，暂用 npm install（建议提交 lockfile）"
