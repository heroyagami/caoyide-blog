...
function categoryNavbarItem({ text, slug, featured, children: explicitChildren }) {
  ...
  let children;
  if (explicitChildren) {
    // 显式 children（如刑法有 criminal-law/ 和 amendment/ 两个子路径）—— 直接用
    ...
    // featured：从 category/{slug}.md 解析链接
    children = featured.map((lawName) => {
